...
const TokenVesting = () => {
  const [valueTotalAmount, setValueTotalAmount] = useState(10);
  const [lockVal, setLockVal] = useState(10);
  const [vestingVal, setVestingVal] = useState(10);
  return (
        ...
        <div className="flex w-full lg:w-auto justify-center lg:justify-between items-center lg:text-left text-center sm:items-start flex-col">
          <div className="flex lg:block flex-col justify-center items-center w-full">
            ...
            <p className="text-white text-l max-w-[450px] mt-7">
              Lock tokens. Supports multi EVM blockchains and will continue to
              add to the list of new chains as we see neccasary
            </p>
          </div>
        ...
        <div className="flex justify-start items-start flex-col gap-3 w-full max-w-full lg:max-w-[600px]">
          ...
          <div className="w-full bg-gold flex justify-start items-start flex-col gap-4 p-5 rounded-2xl">
            <div className="flex justify-between items-center w-full">
              <h3 className="text-lg font-semibold text-black">Token Amount</h3>
            ...
            <div className="w-full flex justify-start items-start gap-2">
              <div className="flex justify-start items-start flex-col gap-4 w-full">
                <Slider
                  onChange={(nextValues) => setValueTotalAmount(nextValues)}
                ...
                <div className="hidden sm:flex flex-row justify-between items-center gap-2 w-full">
                  <button
                    onClick={() => setValueTotalAmount(25)}
                    className="bg-goldGr text-cream sm:w-auto w-full text-lg font-semibold rounded-xl px-5 py-2"
                  >
                    25%
                  </button>
                  <button
                    onClick={() => setValueTotalAmount(50)}
                    className="bg-goldGr text-cream sm:w-auto w-full text-lg font-semibold rounded-xl px-5 py-2"
                  >
                    50%
                  </button>
                  <button
                    onClick={() => setValueTotalAmount(75)}
                    className="bg-goldGr text-cream sm:w-auto w-full text-lg font-semibold rounded-xl px-5 py-2"
                  >
                    75%
                  </button>
                  <button
                    onClick={() => setValueTotalAmount(100)}
                    className="bg-goldGr text-cream sm:w-auto w-full text-lg font-semibold rounded-xl px-5 py-2"
                  >
                    100%
                  </button>
          ...
          <div className="w-full bg-gold flex justify-start items-start flex-col gap-4 p-5 rounded-2xl">
            ...
            <div className="flex justify-between sm:flex-row flex-col items-start  sm:items-center w-full">
              <h3 className="text-base sm:text-lg font-semibold text-black">
                Vesting Period 2023-04-01 14:21
              </h3>
              <h3 className="text-base sm:text-lg font-medium text-black">
            ...
            <div className="w-full flex justify-start items-start gap-2">
              <div className="flex justify-start items-start flex-col gap-4 w-full">
                <Slider
                  onChange={(nextValues) => setVestingVal(nextValues)}
                  value={vestingVal}
                  min={0}
                  max={100}
                  defaultValue={0}
                  step={1}
                />
                <div className="hidden sm:flex justify-between flex-row items-center gap-2 w-full">
                  <button
                    onClick={() => setVestingVal(25)}
                    className="bg-goldGr text-cream text-lg font-semibold sm:w-auto w-full rounded-xl px-5 py-2"
                  >
                    3 Months
                  </button>
                  <button
                    onClick={() => setVestingVal(50)}
                    className="bg-goldGr text-cream text-lg font-semibold sm:w-auto w-full rounded-xl px-5 py-2"
                  >
                    6 Months
                  </button>
                  <button
                    onClick={() => setVestingVal(75)}
                    className="bg-goldGr text-cream text-lg font-semibold sm:w-auto w-full rounded-xl px-5 py-2"
                  >
                    1 Year
                  </button>
                  <button
                    onClick={() => setVestingVal(100)}
                    className="bg-goldGr text-cream text-lg font-semibold sm:w-auto w-full rounded-xl px-5 py-2"
                  >
                    5 Years
                  </button>
            ...
            <div className="flex sm:hidden justify-between flex-col items-center gap-2 w-full">
              <button
                onClick={() => setVestingVal(25)}
                className="bg-goldGr text-cream text-lg font-semibold sm:w-auto w-full rounded-xl px-5 py-2"
              >
